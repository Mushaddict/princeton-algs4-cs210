...
public class Term implements Comparable<Term> {
    ...
    private static class PrefixOrder implements Comparator<Term> {
        ...
        public int compare(Term v, Term w) {
            ...
            if (v.query.length() <= r) {
                if (w.query.length() <= r) {
                    //both v and w is belong to r.length
                    return v.query.compareTo(w.query);
                } else {
                    //w has more r characters 
                    return v.query.compareTo(w.query.substring(0, r));
                }
            } else if (w.query.length() <= r) {
                //v has more characters than r
                return v.query.substring(0, r).compareTo(w.query);
            } else {
                //last case
                return v.query.substring(0, r).
                    compareTo(w.query.substring(0, r));
            }
        }
    }
